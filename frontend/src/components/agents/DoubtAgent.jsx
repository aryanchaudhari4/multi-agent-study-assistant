...
const chips = ["Why does ice float?", "Newton's 2nd Law", 'Photosynthesis', 'DNA vs RNA', 'Integration by parts']

export default function DoubtAgent({ dark }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '14px 20px', borderBottom: dark ? '1px solid rgba(255,255,255,0.06)' : '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: 12, background: dark ? '#0a0a0f' : 'white', flexShrink: 0 }}>
        <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>💡</div>
        <div>
          <div style={{ fontWeight: 800, fontSize: 14, color: dark ? 'white' : '#111', letterSpacing: '-0.02em' }}>Doubt Solver</div>
          <div style={{ fontSize: 12, color: dark ? 'rgba(255,255,255,0.35)' : '#999' }}>Step-by-step explanations for any concept</div>
        </div>
        <div style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 100, background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.2)', color: '#fb923c', display: 'flex', alignItems: 'center', gap: 5 }}>
          <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#fb923c', boxShadow: '0 0 5px #fb923c', display: 'inline-block' }} />
          active
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '20px' }}>
            <div style={{ maxWidth: 520, width: '100%', textAlign: 'center', background: dark ? 'rgba(255,255,255,0.03)' : 'white', border: dark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #ebebeb', borderRadius: 24, padding: '36px 32px', boxShadow: dark ? '0 20px 60px rgba(0,0,0,0.4)' : '0 8px 30px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ fontWeight: 900, fontSize: 20, color: dark ? 'white' : '#111', letterSpacing: '-0.04em', marginBottom: 8 }}>Doubt Solver</div>
              <div style={{ fontSize: 14, color: dark ? 'rgba(255,255,255,0.4)' : '#999', marginBottom: 24, lineHeight: 1.6 }}>
                Ask me anything — I'll break it down clearly with step-by-step explanations and real examples.
              </div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 20 }}>
                {chips.map(chip => (
                  <button key={chip} onClick={() => sendMessage(chip)}
                    style={{ padding: '8px 16px', borderRadius: 100, cursor: 'pointer', fontSize: 13, fontWeight: 600, background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.2)', color: '#fb923c', transition: 'all 0.15s' }}
              ...
              <div style={{ fontSize: 12, color: dark ? 'rgba(255,255,255,0.2)' : '#ccc' }}>or type your doubt below ↓</div>
            </div>
          </div>
        )}
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            ...
            <div style={{ padding: '12px 16px', borderRadius: '4px 16px 16px 16px', background: dark ? 'rgba(255,255,255,0.05)' : '#f4f4f5', border: dark ? '1px solid rgba(255,255,255,0.07)' : '1px solid #e4e4e7' }}>
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                {[0,1,2].map(i => <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: dark ? 'rgba(255,255,255,0.3)' : '#ccc', animation: 'doubtBounce 1.2s infinite', animationDelay: `${i*0.15}s` }} />)}
              </div>
            </div>
          </div>
        )}
      ...
      <InputBar onSend={sendMessage} loading={loading} placeholder="Type your doubt or question..." dark={dark} />
      <style>{`@keyframes doubtBounce { 0%,80%,100%{transform:translateY(0);opacity:0.4} 40%{transform:translateY(-5px);opacity:1} }`}</style>
    </div>
  )
}