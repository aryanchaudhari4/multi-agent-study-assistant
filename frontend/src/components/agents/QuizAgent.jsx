    ...
    <div className="flex flex-col h-full">
      <div className={`px-6 py-4 border-b ${dark ? 'border-gray-800' : 'border-gray-200'}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className={`font-semibold ${dark ? 'text-white' : 'text-gray-900'}`}>Quiz Agent</h2>
            <p className={`text-xs ${dark ? 'text-gray-500' : 'text-gray-400'}`}>Generates MCQs and practice tests</p>
          </div>
          <span className="ml-auto text-xs px-2.5 py-1 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30">active</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-4">
        {messages.length === 0 && (
          <div className="text-center mt-10">
            ...
            <p className={`text-sm font-medium mb-1 ${dark ? 'text-gray-300' : 'text-gray-700'}`}>Quiz Agent ready!</p>
            <p className={`text-xs mb-4 ${dark ? 'text-gray-500' : 'text-gray-400'}`}>Tell me a topic and I'll quiz you on it</p>
            <div className="flex flex-wrap gap-2 justify-center">
              {chips.map(chip => (
                <button key={chip} onClick={() => sendMessage(`Give me 3 MCQs on ${chip}`)}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition border ${dark ? 'bg-blue-500/10 text-blue-400 border-blue-500/30 hover:bg-blue-500/20' : 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100'}`}>
                  {chip}
                </button>
              ))}
            </div>
